...
function log.notify(scope, level, verbose, str, ...)
  if not verbose and _G.NeotestBun.config ~= nil and not _G.NeotestBun.config.debug then
    return
  end
  ...
  vim.notify(
    string.format("[neotest-bun.nvim@%s] %s", scope, string.format(str, ...)),
    level,
    { title = "neotest-bun.nvim" }
  )
end
...
function log.warn_deprecation(options)
  ...
  if uses_deprecated_option then
    ...
    log.notify(
      ...
      vim.log.levels.WARN,
      true,
      "use `:h NeotestBun.options` to read more."
    )
  end
end
